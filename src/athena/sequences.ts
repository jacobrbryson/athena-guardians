...
/* -------------------------------------------------------------------------- */
/* First-contact onboarding openers                                           */
/*                                                                            */
/* After the greeting, Athena initiates a short interactive exchange so the   */
/* Guardian feels they're talking to a real character — not landing in a      */
/* blank chat. These opener lines are scripted (exact, instant, cinematic);   */
/* Athena's *responses* to the Guardian's reply are generated by the live AI  */
/* in companion mode (see core_api prompt builder). Blank lines render as     */
/* gaps (the bubble uses `whitespace-pre-line`) and pace the spoken delivery. */
/* -------------------------------------------------------------------------- */

/** New Guardian: Athena's opening "is this channel working?" prompt. */
export const NEW_GUARDIAN_PROMPT = [
  'Before we begin…',
  "I'd like to know that our communication channel is working.",
  'You can type, or simply speak.',
  'Say hello.',
].join('\n\n');

/** Returning Guardian: Athena's "did you bring your notebook?" prompt. */
